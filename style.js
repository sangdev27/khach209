...
function playParticle(p) {
  ...
  // Tính điểm đích ngẫu nhiên xung quanh cây thông
  var angle = gsap.utils.random(0, Math.PI * 2);
  var distance = gsap.utils.random(40, 160);
  var targetX = startX + Math.cos(angle) * distance;
  var targetY = startY + Math.sin(angle) * distance + gsap.utils.random(20, 80);
...
// Function để bắt đầu animation cây thông
window.startTreeAnimation = function() {
  ...
  gsap.to(".mainSVG", {
    ...
    opacity: 0,
    onComplete: function () {
      if (typeof startTextEffect === "function") {
        startTextEffect();
      }
    }
  });
